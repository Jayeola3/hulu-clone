  ...
  const SearchMovie = async(e)=>{
	...
	return (
      ...
      <section className="flex px-10 sm:px-20 text-2xl whitespace-nowrap space-x-10 sm:space-x-20 overflow-x-scroll scrollbar-hide">
        {requests.map((todo) =>{
          return(
            <div><h3 className="last:pr-24 cursor-pointer transition duration-100 transform hover:scale-125 hover:text-white active:text-red-500" key={todo.url} value={todo.url} onClick={changeSelect}>{todo.title}</h3></div>
          )
        })}
      </section>
